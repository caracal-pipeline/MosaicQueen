                    ...
                    if Rfits[0].header['naxis2'] != imR_naxis2:
                        ...
                        if dcrpix2 > 0:
                            if Rfits[0].header['naxis'] == 2:
                                ...
                                Rfits[0].data = Rfits[0].data[:,:,dcrpix2:,:]
                            Rfits[0].header['naxis2'] -= dcrpix2
                            Rfits[0].header['crpix2'] -= dcrpix2
                        # case where pbR misses some Y's at the start of the array
                        elif dcrpix2 < 0:
                            if Rfits[0].header['naxis'] == 2:
                                Rfits[0].data = np.concatenate([np.empty((-dcrpix2, Rfits[0].header['naxis1'])) * np.nan, Rfits[0].data], axis=1)
                            if Rfits[0].header['naxis'] == 3:
                                Rfits[0].data = np.concatenate([np.empty((Rfits[0].header['naxis3'], -dcrpix2, Rfits[0].header['naxis1'])) * np.nan, Rfits[0].data], axis=2)
                            if Rfits[0].header['naxis'] == 4:
                                Rfits[0].data = np.concatenate([np.empty((Rfits[0].header['naxis4'], Rfits[0].header['naxis3'], -dcrpix2, Rfits[0].header['naxis1'])) * np.nan, Rfits[0].data], axis=3)
                            Rfits[0].header['naxis2'] -= dcrpix2
                            Rfits[0].header['crpix2'] -= dcrpix2
                        # check end of array
                        ...
                        # case where pbR misses some Y's at the end of the array
                            ...
                            if Rfits[0].header['naxis'] == 2:
                                Rfits[0].data = np.concatenate([Rfits[0].data, np.empty((-dnaxis2, Rfits[0].header['naxis1'])) * np.nan], axis=1)
                            if Rfits[0].header['naxis'] == 3:
                                Rfits[0].data = np.concatenate([Rfits[0].data, np.empty((Rfits[0].header['naxis3'], -dnaxis2, Rfits[0].header['naxis1'])) * np.nan], axis=2)
                            if Rfits[0].header['naxis'] == 4:
                                Rfits[0].data = np.concatenate([Rfits[0].data, np.empty((Rfits[0].header['naxis4'], Rfits[0].header['naxis3'], -dnaxis2, Rfits[0].header['naxis1'])) * np.nan], axis=3)
                            Rfits[0].header['naxis2'] -= dnaxis2
                        modified_head = True
                    if modified_head:
